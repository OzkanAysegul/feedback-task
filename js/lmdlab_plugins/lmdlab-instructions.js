 ...
 *
 *
 * Modification taken from Poldrack lab plugins by Elliott Wimmer - changed navigation buttons such that the "Next" button reads
 * "End Instructions" on the last page
 */


jsPsych.plugins['lmdlab-instructions'] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'lmdlab-instructions',
    description: '',
    parameters: {
      pages: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    function show_current_page() {
      ...
      if (trial.show_clickable_nav) {
        ...
        if (trial.pages.length == 1) {
						nav_html += "<button id='jspsych-instructions-next' class='lmdlab-btn'"+
						"style='margin-left: 5px;'>End Instructions"+
						" &gt;</button></div>";
        } else{
					...
					if (trial.allow_backward) {
						...
						if (current_page > 0){
							nav_html += "<button id='jspsych-instructions-back' class='lmdlab-btn' style='margin-right: 5px;' "+allowed+">&lt; "+trial.button_label_previous+"</button>";
						}
					}
    ...
    function endTrial() {
      ...
      var trial_data = {
      // NOT saving view history here - GEW 2023
